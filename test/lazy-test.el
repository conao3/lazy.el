...
(ert-deftest lazy-test-from-seq ()
  (should (equal (lazy-into-list (lazy-from-seq '(1 2 3)))
                 '(1 2 3)))
  (should (equal (lazy-into-list (lazy-from-seq [1 2 3]))
                 '(1 2 3)))
  (should (equal (lazy-into-list (lazy-from-seq "abc"))
                 '(?a ?b ?c)))
  (should (equal (lazy-into-list (lazy-from-seq '(1 2 3 4 5) (lambda (x) (< x 4))))
                 '(1 2 3)))
  (should (equal (lazy-into-list (lazy-from-seq [10 20 30 40] (lambda (x) (< x 35))))
                 '(10 20 30)))
  (should (equal (lazy-into-list (lazy-from-seq "hello" (lambda (c) (not (eq c ?l)))))
                 '(?h ?e))))
